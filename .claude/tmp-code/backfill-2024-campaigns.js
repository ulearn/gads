...
(async () => {
  ...
  console.log('\n📅 BACKFILLING 2024 CAMPAIGN ATTRIBUTION\n');
  console.log('='.repeat(80));

  // Get all 2024 contacts with NULL campaign that have hsa_cam
  const [contacts] = await conn.execute(`
    ...
    WHERE hs_analytics_source = 'PAID_SEARCH'
      ...
      AND createdate >= '2024-01-01' AND createdate < '2025-01-01'
      AND hs_analytics_first_url LIKE '%hsa_cam=%'
  `);

  console.log(`\n📊 Found ${contacts.length} contacts from 2024 with campaign IDs\n`);
