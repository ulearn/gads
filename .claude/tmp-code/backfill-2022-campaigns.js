...
(async () => {
  ...
  console.log('\n📅 BACKFILLING OCT-DEC 2022 CAMPAIGN ATTRIBUTION\n');
  console.log('='.repeat(80));

  // Get all Oct-Dec 2022 contacts with NULL campaign that have hsa_cam
  const [contacts] = await conn.execute(`
    ...
    WHERE hs_analytics_source = 'PAID_SEARCH'
      ...
      AND createdate >= '2022-10-01' AND createdate < '2023-01-01'
      AND hs_analytics_first_url LIKE '%hsa_cam=%'
  `);

  console.log(`\n📊 Found ${contacts.length} contacts from Oct-Dec 2022 with campaign IDs\n`);
  ...
  console.log('='.repeat(80));
  console.log('\n📈 MYSQL UPDATE SUMMARY (Oct-Dec 2022):\n');
  ...
  if (hubspotUpdates.length > 0) {
    console.log('\n' + '='.repeat(80));
    console.log('\n🔄 UPDATING HUBSPOT (Oct-Dec 2022):\n');

    const result = await updateHubSpotContacts(null, hubspotUpdates);

    console.log('\n📈 HUBSPOT UPDATE SUMMARY (Oct-Dec 2022):\n');
